...
def _as_point(obj: dict) -> dict | None:
    ...
    try:
        ...
        py = float(y)
        pz = float(z)
        pt = int(float(ts)) if ts is not None else None
    except Exception:
        return None
    result = {"x": px, "y": py, "z": pz}
    if pt is not None:
        result["timestamp"] = pt
    return result
...
def _iter_points_from_csv(path: str):
    def _lower_fields(fields):
        return [h.strip().lower() for h in fields] if fields else []

    with open(path, "r", encoding="utf-8") as f:
        try:
            # Try DictReader with headers
            reader = csv.DictReader(f)
            headers = _lower_fields(reader.fieldnames)
            # If fieldnames are None, there was no header; fall back below
            if headers:
                # Case 1: x,y,z,(timestamp)
                if any(h in ("x", "y", "z", "timestamp") for h in headers):
                    for row in reader:
                        pt = _as_point(row)
                        if pt:
                            yield pt
                ...
                # Case 2: polar headers like Distance_cm, Azimuth_deg, Elevation_deg
                def pick(keys: list[str], row: dict):
                    for k in keys:
                        if k in row:
                            return row[k]
                ...
                polar_detect = any(h in ("distance_cm", "range_cm", "distance_m", "range_m") for h in headers) and \
                                any(h in ("azimuth_deg", "az_deg", "azimuth") for h in headers) and \
                                any(h in ("elevation_deg", "el_deg", "elevation") for h in headers)
                if polar_detect:
                    for row in reader:
                        r_cm = pick(["distance_cm", "range_cm"], row)
                        r_m = pick(["distance_m", "range_m"], row)
                        az = pick(["azimuth_deg", "az_deg", "azimuth"], row)
                        el = pick(["elevation_deg", "el_deg", "elevation"], row)
                        try:
                            rng_m = float(r_m) if r_m is not None else float(r_cm) / 100.0
                            az_deg = float(az)
                            el_deg = float(el)
                            x, y, z = spherical_to_cartesian(rng_m, az_deg, el_deg)
                        except Exception:
                            continue
                        yield {"x": float(x), "y": float(y), "z": float(z)}
                    return
        except Exception:
            pass
...
async def stream_points_from_file(path: str, hz: float, loop_file: bool = True):
    ...
    while True:
        ...
        for pt in _iter_points(path):
            any_sent = True
            if "timestamp" not in pt:
                pt = {**pt, "timestamp": int(time.time() * 1000)}
            yield pt
            if delay:
