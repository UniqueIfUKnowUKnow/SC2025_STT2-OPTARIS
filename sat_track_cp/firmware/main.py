                    ...
                    if anomaly_found:
                        ...
                        try:
                            ...
                            if geodetic_to_ecef and lidar_spherical_to_ecef_delta:
                                ...
                                # Fallback: simple local Cartesian (km) in sensor frame
                                dist_km = distance_m / 1000.0
                                az_rad = np.radians(az_deg)
                                el_rad = np.radians(el_deg)
                                ce = np.cos(el_rad)
                                x_km = float(dist_km * ce * np.cos(az_rad))
                                y_km = float(dist_km * ce * np.sin(az_rad))
                                z_km = float(dist_km * np.sin(el_rad))

                            # Write last detected point to shared JSON for external clients
                            try:
                                base_dir = os.path.dirname(os.path.abspath(__file__))
                                last_point_path = os.path.abspath(os.path.join(base_dir, '..', 'scripts', 'last_point.json'))
                                payload = {
                                    "x": float(x_km) if x_km is not None else None,
                                    "y": float(y_km) if y_km is not None else None,
                                    "z": float(z_km) if z_km is not None else None,
                                    "timestamp": int(ts_ms),
                                    "distance_m": float(distance_m),
                                    "azimuth_deg": float(az_deg),
                                    "elevation_deg": float(el_deg)
                                }
                                with open(last_point_path, 'w') as f:
                                    json.dump(payload, f)
                            except Exception as file_err:
                                print(f"[WS] Could not write last_point.json: {file_err}")
