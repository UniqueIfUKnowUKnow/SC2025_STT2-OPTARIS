# --- Standard Library Imports ---
import RPi.GPIO as GPIO  # For direct control of stepper motor GPIO pins
import pigpio            # For stable, hardware-based PWM for the servo
import time              # For creating delays
import serial            # For reading data from the LiDAR's serial port
import threading         # To run the LiDAR reader in the background
import queue             # For thread-safe data sharing between threads
import numpy as np
from lidar_reader import LidarReader
from constants import *
from stepper_setup import setup_stepper_gpio
from move_motors import *
from calibration import *
from tle_processing import parse_tle
from anomaly_check import get_interpolated_reference_distance
from scanning import perform_scanning_sequence
from datetime import datetime
from kalman_filter import DroneTrajectoryKalman
from tracking_functions import TrackingMetrics, MultiTargetTracker, perform_tracking_detection, create_tracking_dashboard, save_tracking_state, load_tracking_state, export_tracking_data, adaptive_tracking_parameters


# --- Main Application ---
def main():
    """
    The main entry point of the program. It handles setup, state management,
    and the main control loop for the calibration and scanning process.
    """
    # --- Setup ---
    setup_stepper_gpio()
    pi = pigpio.pi()
    if not pi.connected:
        print("Could not connect to pigpio daemon. Is it running?")
        return
    GPIO.output(DIR_PIN, GPIO.HIGH)
    

    servo_angle = SERVO_SWEEP_START
    set_servo_angle(pi, servo_angle)


    lidar_data_queue = queue.Queue()
    lidar_thread = LidarReader(LIDAR_SERIAL_PORT, LIDAR_BAUD_RATE, lidar_data_queue)
    lidar_thread.start()

    kf = DroneTrajectoryKalman()

    tle_data = parse_tle([satellite_name, line1, line2])
    
    # --- State Machine and Variables ---
    states = ["CALIBRATING", "SCANNING", "DETECTED"]
    current_state = states[0]
    stepper_steps = 0
    current_azimuth = 0
    current_elevation = 0
    anomaly_detected = False
    anomaly_locations = []
    
    anomaly_averaged_coords = []
    initial_rad = []
    first_scan_positions = []
    first_scan_timestamps = []
    plot_data = []
    locked_in = 0
    anomaly_count = 0
    anomaly_total = 0
    
    # Initialize enhanced tracking capabilities
    tracking_metrics = TrackingMetrics()
    multi_target_tracker = MultiTargetTracker(max_targets=3)
    
    # Load previous tracking state if available
    previous_state = load_tracking_state()
    if previous_state:
        print("Loaded previous tracking state")
        # Could restore some metrics here if needed
    
    # Base tracking parameters
    base_tracking_params = {
        'detection_threshold_factor': ANOMALY_FACTOR,
        'max_samples': 50,
        'search_radius_deg': 5.0
    }

    try:
        while True:
            if current_state == "CALIBRATING":
                print("Calibrating sensors...")

                # Mapping environment
                calibration_data = calibrate_environment(pi, lidar_data_queue)
                print(calibration_data)
                save_calibration_data(calibration_data)

                # Move to a reasonable elevation for tracking (not maximum)
                # Use middle of the elevation range instead of maximum
                reasonable_elevation = (SERVO_SWEEP_START + SERVO_SWEEP_END) / 2
                print(f"Moving to reasonable elevation for tracking: {reasonable_elevation:.1f}° (range: {SERVO_SWEEP_START}°-{SERVO_SWEEP_END}°)")
                
                current_azimuth, current_elevation, stepper_steps = move_to_polar_position(
                    pi, tle_data["arg_perigee_deg"], reasonable_elevation, stepper_steps
                )
                
                print(f"Calibration complete. Position: Az={current_azimuth:.1f}°, El={current_elevation:.1f}°")

                calibration_done = True
                if calibration_done:
                    current_state = states[1]

            # Corrected scanning section for main.py

            elif current_state == "SCANNING":
                print("Scanning area...")
                
                
                #Sweeping for points
                current_azimuth, current_elevation, stepper_steps, anomaly_averaged_coords, anomaly_count, calibration_done = perform_scanning_sequence(
                    pi, lidar_data_queue, calibration_data, current_azimuth, current_elevation, 
                    stepper_steps, anomaly_locations, anomaly_averaged_coords, anomaly_count, 5
                )

                if calibration_done:
                    current_state = states[2]

            elif current_state == "DETECTED":
                anomaly_count = 0
                coords_array = np.array([list(coord_tuple[0]) for coord_tuple in anomaly_averaged_coords])
                first_scan_positions = coords_array[:, :3]
                first_scan_timestamps = coords_array[:, 3:].flatten()
                
                # Convert degrees to radians for Kalman filter
                measurements_for_kalman = []
                for i, pos in enumerate(first_scan_positions):
                    dist, az_deg, el_deg = pos[0], pos[1], pos[2]
                    # Convert to radians
                    az_rad = np.radians(az_deg)
                    el_rad = np.radians(el_deg)
                    measurements_for_kalman.append([dist, az_rad, el_rad])
                
                # DEBUG: Print converted measurements
                print("=== KALMAN FILTER DEBUG ===")
                print("Measurements in degrees:")
                for i, pos in enumerate(first_scan_positions):
                    print(f"  {i}: [{pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f}] at t={first_scan_timestamps[i]:.3f}")
                
                print("Measurements in radians for Kalman:")
                for i, pos in enumerate(measurements_for_kalman):
                    print(f"  {i}: [{pos[0]:.1f}, {pos[1]:.3f}, {pos[2]:.3f}]")
                
                # Initialize Kalman filter
                kf.process_measurement_sequence(measurements_for_kalman, first_scan_timestamps)
                
                # DEBUG: Check initial state
                kf.debug_velocities()
                
                # Test prediction
                test_future_time = first_scan_timestamps[-1] + DT
                test_prediction = kf.predict_future_positions([test_future_time])
                pred_deg = kf.convert_prediction_to_degrees(test_prediction[0])
                print(f"Test prediction (+1s): dist={pred_deg[0]:.1f}m, az={pred_deg[1]:.1f}Â°, el={pred_deg[2]:.1f}Â°")
                print("=== END DEBUG ===\n")

                print("Target detected! Starting EKF tracking...")
                
                # Import the tracking function
                from tracking_functions import perform_tracking_detection
                
                # Clear LiDAR queue before starting
                while not lidar_data_queue.empty():
                    try:
                        lidar_data_queue.get_nowait()
                    except queue.Empty:
                        break

                max_tracking_cycles = 20

                while locked_in < max_tracking_cycles:
                    # Use current time for prediction
                    current_time = time.time()
                    future_time = current_time + DT
                    
                    # Get adaptive tracking parameters based on performance
                    current_tracking_params = adaptive_tracking_parameters(tracking_metrics, base_tracking_params)
                    
                    predicted_positions = kf.predict_future_positions([future_time])
                    predicted_position = predicted_positions[0]  # [distance, azimuth_rad, elevation_rad]
                    
                    # Convert prediction to degrees for motor control
                    pos_deg = kf.convert_prediction_to_degrees(predicted_position)
                    
                    # Validate elevation prediction is within servo limits
                    if pos_deg[2] < SERVO_SWEEP_START or pos_deg[2] > SERVO_SWEEP_END:
                        print(f"WARNING: Predicted elevation {pos_deg[2]:.1f}° is outside servo range!")
                        print(f"Clamping elevation from {pos_deg[2]:.1f}° to valid range {SERVO_SWEEP_START}°-{SERVO_SWEEP_END}°")
                        pos_deg[2] = max(SERVO_SWEEP_START, min(SERVO_SWEEP_END, pos_deg[2]))
                        print(f"Adjusted elevation to: {pos_deg[2]:.1f}°")
                    
                    # Check if servo is stuck at extreme elevation
                    if abs(current_elevation - SERVO_SWEEP_END) < 2.0 and abs(pos_deg[2] - current_elevation) > 10.0:
                        print(f"WARNING: Servo appears stuck at maximum elevation {current_elevation:.1f}°")
                        print("Attempting to reset servo to safe position...")
                        from tracking_functions import reset_servo_to_safe_position
                        safe_elevation = reset_servo_to_safe_position(pi)
                        current_elevation = safe_elevation
                        print(f"Servo reset to safe elevation: {safe_elevation:.1f}°")
                    
                    print(f"Prediction: Az={pos_deg[1]:.1f}°, El={pos_deg[2]:.1f}°")
                    print(f"Current position: Az={current_azimuth:.1f}°, El={current_elevation:.1f}°")
                    print(f"Tracking parameters: threshold={current_tracking_params['detection_threshold_factor']:.2f}, samples={current_tracking_params['max_samples']}")
                    
                    # Debug elevation movement if there's a significant change
                    if abs(pos_deg[2] - current_elevation) > 5.0:  # More than 5° change
                        from tracking_functions import debug_elevation_movement
                        debug_elevation_movement(pi, current_elevation, pos_deg[2], SERVO_PIN)
                    
                    # Use enhanced tracking detection with adaptive search
                    new_measurement, current_azimuth, current_elevation, stepper_steps, detection_confidence = perform_tracking_detection(
                        pi, lidar_data_queue, calibration_data, pos_deg[1], pos_deg[2], stepper_steps,
                        tracking_metrics=tracking_metrics, adaptive_search=True
                    )

                    # Process new measurement if found
                    if new_measurement is not None:
                        anomaly_total += 1
                        
                        # Extract measurement data: [distance, azimuth, elevation, timestamp]
                        current_measurement_deg = [new_measurement[0], new_measurement[1], new_measurement[2]]
                        
                        # Convert to radians for Kalman filter
                        current_measurement_rad = kf.convert_measurement_to_radians(current_measurement_deg)
                        measurement_time = new_measurement[3]  # Use the timestamp from the measurement
                        
                        # Update Kalman filter with the measurement at its actual time
                        kf.update_with_measurement_at_time(current_measurement_rad, measurement_time)
                        
                        plot_data.append((current_measurement_deg[0], current_measurement_deg[1], current_measurement_deg[2]))
                        print(f"Tracking cycle {locked_in + 1}: Updated with measurement at ({current_measurement_deg[1]:.1f}°, {current_measurement_deg[2]:.1f}°) - Confidence: {detection_confidence:.1f}%")
                        
                        # Debug the updated state
                        kf.debug_velocities()
                        
                        # Display tracking dashboard every 5 cycles
                        if (locked_in + 1) % 5 == 0:
                            dashboard = create_tracking_dashboard(tracking_metrics, multi_target_tracker, plot_data)
                            print(dashboard)
                    else:
                        print(f"Tracking cycle {locked_in + 1}: No target detected at predicted position")
                    
                    locked_in += 1
                
                print("Tracking complete. Saving trajectory data and tracking state...")
                save_calibration_data(plot_data)
                
                # Save tracking state and export data
                save_tracking_state(tracking_metrics, multi_target_tracker)
                export_tracking_data(tracking_metrics, multi_target_tracker, plot_data)
                
                # Display final tracking summary
                from tracking_functions import get_tracking_summary
                final_summary = get_tracking_summary(tracking_metrics, multi_target_tracker)
                print(final_summary)
                
                
    except KeyboardInterrupt:
        print("\nProgram stopped by user.")
    finally:
        print("Cleaning up...")
        pi.set_servo_pulsewidth(SERVO_PIN, 0)
        reset_stepper_pos(stepper_steps)
        pi.stop()
        GPIO.cleanup()
        

if __name__ == '__main__':
    main()