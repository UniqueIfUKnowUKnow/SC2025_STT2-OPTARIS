#!/usr/bin/env python3
"""
Convert polar coordinate CSV to Cartesian coordinates for the WebSocket server.
"""

import csv
import math

def polar_to_cartesian(distance_cm, azimuth_deg, elevation_deg):
    """Convert polar coordinates to Cartesian (x, y, z) in meters."""
    ...
    z = range_m * cos_el * math.sin(az_rad)
    y = range_m * math.sin(el_rad)
    
    return x, y, z

def convert_csv(input_file, output_file):
    """Convert polar CSV to Cartesian CSV."""
    with open(input_file, 'r') as infile, open(output_file, 'w', newline='') as outfile:
        ...
        for row in reader:
            try:
                ...
                x, y, z = polar_to_cartesian(distance, azimuth, elevation)
                
                # Convert timestamp to milliseconds
                timestamp_ms = int(time_s * 1000)
                
                # Write converted data
                writer.writerow([x, y, z, timestamp_ms])
...
if __name__ == "__main__":
    # Save your data to 'polar_data.csv' first, then run:
    convert_csv('polar_data.csv', 'cartesian_data.csv')
    print("Conversion complete! Use 'cartesian_data.csv' with the WebSocket server.")
    
    # Example of what the output would look like for your first few rows:
    print("\nExample converted data:")
    sample_data = [
        (126.7, 0.96, 5.24, 1755940504.670),
        (148.7, 10.91, 6.19, 1755940505.000),
        (114.5, 20.87, 1.33, 1755940505.510),
    ]
    
    for dist, az, el, time_s in sample_data:
        x, y, z = polar_to_cartesian(dist, az, el)
        print(f"Distance: {dist}cm, Az: {az}°, El: {el}° → x: {x:.3f}, y: {y:.3f}, z: {z:.3f}")
